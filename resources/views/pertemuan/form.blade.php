...
<div class="my-7 mx-auto bg-white sm:w-[94%] w-4/5 rounded-2xl item h-fit p-5 shadow-md">
    ...
    <form action="{{ route('pertemuan.store') }}" method="POST">
        ...
        <div class="mt-11 w-full">
            ...
            <div class="mt-3">
                <p class="font-semibold text-base text-non-active mb-2">Guru</p>
                <select {{ isset($data) ? 'disabled' : '' }} name="guru_id" class="input-select">
                    <option value="" selected hidden>Pilih Jenis Karir</option>
                    @foreach ($gurus as $guru)
                    <option value='{{ $guru->id }}' {{ isset($data) ? ($data->guru_id == $guru->id ? "selected" : "") : '' }}>{{ $guru->user->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mt-3">
                <p class="font-semibold text-base text-non-active mb-2">Murid</p>
                <select {{ isset($data) ? 'disabled' : '' }} name="user_id" class="input-select">
                    <option value="" selected hidden>Pilih Jenis Karir</option>
                    @foreach ($siswas as $siswa)
                    <option value='{{ $siswa->id }}' {{ isset($data) ? ($data->user_id == $siswa->id ? "selected" : "") : '' }}>{{ $siswa->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mt-3">
